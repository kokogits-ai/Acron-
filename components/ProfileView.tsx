    ...
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row items-center gap-8 bg-white p-8 rounded-3xl border border-gray-100 shadow-sm">
        ...
        <div className="text-center md:text-left">
          ...
          <p className="text-gray-500 font-medium mb-4">Acron Private Banking US since 2018</p>
          <div className="flex flex-wrap justify-center md:justify-start gap-3">
             <span className="bg-blue-100 text-blue-700 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider">Elite Client</span>
             ...
             <span className="bg-emerald-100 text-emerald-700 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider">Tax Resident: USA-{user.state}</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className="bg-white p-8 rounded-3xl border border-gray-100 shadow-sm space-y-6">
          ...
          <div className="space-y-4">
             <ProfileItem icon={Fingerprint} label="Account Number" value={user.accountNumber} mono />
             <ProfileItem icon={BadgeCheck} label="Routing Number" value={user.routingNumber || 'N/A'} mono />
             <ProfileItem icon={Calendar} label="Age" value={`${user.age} Years`} />
             <ProfileItem icon={Calendar} label="Account Type" value={user.accountType || 'Checking'} />
             <ProfileItem icon={Globe} label="Citizenship" value={user.country} />
             <ProfileItem icon={MapPin} label="Address" value={`${user.city}, ${user.state} ${user.zipCode}`} />
          </div>
        </div>
        ...
        <div className="bg-white p-8 rounded-3xl border border-gray-100 shadow-sm space-y-6">
          ...
          <div className="space-y-4">
             ...
             <ProfileItem icon={Phone} label="Verified Phone" value={user.phone || 'N/A'} />
             <ProfileItem icon={Globe} label="Portal Language" value="English (US)" />
